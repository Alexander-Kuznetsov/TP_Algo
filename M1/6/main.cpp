/* «адание 6.2
ƒано N кубиков. 
“ребуетс€ определить каким количеством способов можно 
выстроить из этих кубиков пирамиду.
 аждый вышележащий слой пирамиды должен быть строго меньше нижележащего.
*/
...
unsigned long long getCount(unsigned long long **arr, int size) {
	...
	//«аполнение размещений по главной диагонали1
	for (int i = 1; i < size; ++i) {
		arr[i][i] = 1;
	}
	...
	for (int i = 1; i < size; ++i) {
		for (int j = 1; j < i; ++j) {
			//≈сли количество камней в основании не превышает количества всех камней
			if ((j <= i) && (i <= j * (j + 1) / 2)) { 
				for (int k = 1; k < j; ++k) {
					arr[i][j] += arr[i - j][k]; //—умма размещение камней выше основани€
				}
			}
		}		
	}
	//—умма всевозможных размещений с size элементами
